...
bool Transformations::Initialize(int screenWidth, int screenHeight, HWND hwnd)
{
	...
	// ------------------------------------
	ourShader = new Shader("5.1.transform.vs", "5.1.transform.fs", m_OpenGL);
...
bool Transformations::Frame()
{
    ...
    ourShader->use();
    unsigned int transformLoc = m_OpenGL->glGetUniformLocation(ourShader->ID, "transform");
    m_OpenGL->glUniformMatrix4fv(transformLoc, 1, GL_FALSE, transform.constData());
