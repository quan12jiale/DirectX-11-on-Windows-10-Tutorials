#include "learnopengl/1.getting_started/7.4.camera_class/camera_class.h"
#include "learnopengl/shader.h"

LearnCameraClass::LearnCameraClass()
    : camera(QVector3D(0.0f, 0.0f, 3.0f))
{
}

bool LearnCameraClass::Initialize(int screenWidth, int screenHeight, HWND hwnd)
{
    bool result;
...
void LearnCameraClass::Shutdown()
{
    if (m_OpenGL)
    {
...
void LearnCameraClass::SetMsg(const MSG& msg)
{
    m_msg = msg;
}

void LearnCameraClass::processInput()
{
...
void LearnCameraClass::mouse_callback()
{
    // 检查消息类型是否为鼠标移动消息
...
void LearnCameraClass::scroll_callback()
{
    // 检查消息类型是否为鼠标滚轮消息
