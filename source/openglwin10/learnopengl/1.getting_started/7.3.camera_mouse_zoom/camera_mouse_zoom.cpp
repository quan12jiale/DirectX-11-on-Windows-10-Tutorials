#include "learnopengl/1.getting_started/7.3.camera_mouse_zoom/camera_mouse_zoom.h"
#include "learnopengl/shader.h"

bool CameraMouseZoom::Initialize(int screenWidth, int screenHeight, HWND hwnd)
{
    bool result;
...
bool CameraMouseZoom::Frame()
{
    ...
    projection.perspective(fov, (float)800 / (float)600, 0.1f, 100.0f);
...
void CameraMouseZoom::SetMsg(const MSG& msg)
{
    m_msg = msg;
}

void CameraMouseZoom::processInput()
{
...
void CameraMouseZoom::mouse_callback()
{
    // 检查消息类型是否为鼠标移动消息
...
void CameraMouseZoom::scroll_callback()
{
    // 检查消息类型是否为鼠标滚轮消息
