...
void DiffuseLightWindow::InitializeDiffuseLightVertexIndexData()
{
	...
	m_diffuseLightVertexData[0].position = XMFLOAT4(-1.0f, -1.0f, 0.0f, 1.0f);  // Bottom left.
	m_diffuseLightVertexData[0].texture = XMFLOAT2(0.0f, 1.0f);
	m_diffuseLightVertexData[0].normal = XMFLOAT3(0.0f, 0.0f, -1.0f);

	m_diffuseLightVertexData[1].position = XMFLOAT4(0.0f, 1.0f, 0.0f, 1.0f);  // Top middle.
	m_diffuseLightVertexData[1].texture = XMFLOAT2(0.5f, 0.0f);
	m_diffuseLightVertexData[1].normal = XMFLOAT3(0.0f, 0.0f, -1.0f);

	m_diffuseLightVertexData[2].position = XMFLOAT4(1.0f, -1.0f, 0.0f, 1.0f);  // Bottom right.
	m_diffuseLightVertexData[2].texture = XMFLOAT2(1.0f, 1.0f);
	m_diffuseLightVertexData[2].normal = XMFLOAT3(0.0f, 0.0f, -1.0f);
...
bool DiffuseLightWindow::InitializeDiffuseLight()
{
	...
	m_diffuseLightBuffer.reset(mRhi->newBuffer(QRhiBuffer::Dynamic, QRhiBuffer::UniformBuffer, sizeof(DiffuseLightBufferType)));
	if (!m_diffuseLightBuffer->create())
		return false;
	...
	mShaderBindings->setBindings({
		...
		QRhiShaderResourceBinding::uniformBuffer(2, QRhiShaderResourceBinding::StageFlag::FragmentStage, m_diffuseLightBuffer.get()),
		});
...
void DiffuseLightWindow::RenderDiffuseLight()
{
	...
	DiffuseLightBufferType light_buf;
	light_buf.lightDirection = { 0.0f, 0.0f, 1.0f };
	light_buf.diffuseLightColor = { 1.0f, 1.0f, 1.0f, 1.0f };
	batch->updateDynamicBuffer(m_diffuseLightBuffer.get(), 0, sizeof(DiffuseLightBufferType), &light_buf);
...
void DiffuseLightWindow::onRenderTick()
{
	RenderDiffuseLight();  // 使用光照纹理渲染
}
